    ...
    : NexusApiController
{
    ...
    private const string GET_BY_ID_ROUTE = $"{nameof(CompaniesController)}.GetById";
    //private const string GET_BY_ID_ROUTE = $"{nameof(CompaniesController)}.{nameof(GetById)}";

    //[HttpGet]
    //[ProducesResponseType(StatusCodes.Status200OK)]
    //[ProducesResponseType(StatusCodes.Status500InternalServerError, Type = typeof(ApiResponse))]
    //public async Task<ActionResult<PagedResponse<SearchCompaniesResponse>>> Get([FromQuery] PageSearchRequest request)
    //{
    //    IPagedList<Company> response = await companyQueryRepository.QueryAsync(request.ToPageSearch());
    //    return Paged(typeAdapter.Adapt<IPagedList<SearchCompaniesResponse>>(response));
    //}
